...
public class BeforeUpdateActivity extends AppCompatActivity {
    ...
    public void onPerformBeforeUpdateClick(View view) {
        ...
        cursor = db.rawQuery("select " + COLUMN_ID + ", " + COLUMN_LAST_NAME + ", " + COLUMN_FIRST_NAME + ", "
                + COLUMN_MIDDLE_NAME + ", " + COLUMN_CREATED_AT + " FROM " + CustomDbHelper.TABLE_NAME
                + " WHERE " + CustomDbHelper.COLUMN_LAST_NAME + " like '" + oldLastName + "';", null);
        if (cursor.moveToNext()) {
            Intent intent = new Intent(this, UpdateActivity.class);
            intent.putExtra("oldLastName", cursor.getString(1));
            intent.putExtra("oldName", cursor.getString(2));
            intent.putExtra("oldMiddleName", cursor.getString(3));
